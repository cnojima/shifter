...
var Stack = require('./stack').Stack,
    ...
    registry,
    cpr = require('cpr').cpr,
    shjshint = require('wflint').jshint,
    cprOptions = {
    ...
    defaultLint = shjshint,//SH: jshint defaults
        ...
        callback: function (info) {
            ...
            if (!!lint && lint.length) {
                ...
                lint.forEach(function (item) {
                    counter = counter + 1;
                    log.console['error']('   #' + counter + ': ' + log.color('[' + item.type + ']', 'red') + ' ' + log.color(item.message, 'yellow'));
                    if (item.evidence) {
                        log.console['error']('       ' + String(item.evidence).trim() + log.color(' // line ' + item.line + ', pos ' + item.col, 'white'));
                     }
                });
...
var buildJS = function (mod, name, callback) {
    ...
    var queue = new Queue({
        ...
        usestamp = mod.shifter && mod.shifter.usestamp,//; //SH: use wrapper
        generated = mod.shifter && mod.shifter.generated;
    ...
    if (mod.stamp && !usestamp) {
        if (mod.yuifile) {
            queue.jsstamp({
                strict: strictMode,
                prefix: "@YUIGLOBALVAR@.add('" + modName + "', ",
                postfix: ", '@VERSION@'" + stringify(mod.config).replace(/\"/g, '\'') + ");\n"
            });
        } else {
            queue.jsstamp({
                strict: strictMode,
                prefix: "@YUIGLOBALVAR@.add('" + modName + "', function (@YUIVAR@, NAME) {\n\n",
                postfix: "\n\n}, '@VERSION@'" + stringify(mod.config).replace(/\"/g, '\'') + ");\n"
            });
        }
    }

    //SH: start use wrapper
    if(usestamp) {
      var reqs = JSON.stringify(mod.config.requires || [], null, 4).replace(/\"/g, '\'').replace(/\[(.*?)\]/g,"$1");
      queue.jsstamp({
        strict: strictMode,
        prefix: "@YUIGLOBALVAR@().use(" + reqs + ", function (@YUIVAR@) {\n\n",
        postfix: "\n\n});\n"
      });
    }
    //SH: end use wrapper
    ...
    if (defaultLint && !generated) { //SH: no lint for generated source [soy, etc]
...
var reporter = function (result, config, options) {
