            ...
            util.find(dir, 'build.json', function(err, json) {
                if (json) {
                    ...
                    var dir = path.dirname(json),
                        mod = path.basename(dir),
                        ldr;
                    /*
                    * SH: start [build app loader meta data module]
                    */
                    //log.info('meta changed, shifting yui');
                    //build(path.join(dir, '../yui'));
                    //if (name.indexOf('./meta') === 0) {
                    if (name.indexOf('/meta/') > -1) {
                        ldr = mod.substring(0,mod.indexOf('-')) + '-loader';
                        log.info('meta changed, shifting '+ldr);
                        build(path.join(dir, '..',ldr));
                       /*
                        * SH: end [build app loader meta data module]
                        */
                    } else {
